...
pub fn generate_cs(
    ...
    for i in counter * 2..min(oods_shifted_logsize_26_labels.len(), counter * 2 + 2) {
        ...
        ldm.write(format!("column_line_coeff_{}", oods_shifted_logsize_26_labels[i]), &coeff)?;
    }

    ldm.write(format!("line_coeff_randomizer_26_alpha_{}", counter + 1), &line_coeff_randomizer.alpha)?;
