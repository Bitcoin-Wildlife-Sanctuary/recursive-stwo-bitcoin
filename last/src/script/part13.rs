...
use recursive_stwo_primitives::quotient::{complex_conjugate_line_coeffs_var, LineCoeffRandomizerBar};
...
pub fn generate_cs(
    ...
    let after_sampled_values_random_coeff: QM31Bar = ldm.read("after_sampled_values_random_coeff")?;
    ...
    for i in counter * 2..min(oods_original_logsize_26_labels.len(), counter * 2 + 2) {
        ...
        let alpha = line_coeff_randomizer.get_and_update(&table, &after_sampled_values_random_coeff);
        let coeff = complex_conjugate_line_coeffs_var(
            &table, 
            &oods_point,
            &value,
            &alpha,
        )?;
        ldm.write(format!("column_line_coeff_{}", oods_original_logsize_26_labels[i]), &coeff)?;
    }

    ldm.write(format!("line_coeff_randomizer_26_alpha_{}", counter + 2 + 1), &line_coeff_randomizer.alpha)?;
