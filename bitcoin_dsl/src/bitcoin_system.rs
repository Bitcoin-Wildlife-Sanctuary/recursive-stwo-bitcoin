use crate::bar::{AllocationMode, Bar};
...
/// A shared reference to a bitcoin script system that can be stored in high level
/// variables.
#[derive(Clone, Debug)]
pub struct BitcoinSystemRef(pub(crate) Rc<RefCell<BitcoinSystem>>);

impl PartialEq for &BitcoinSystemRef {
    fn eq(&self, other: &Self) -> bool {
        Rc::ptr_eq(&self.0, &other.0)
    }
}

impl Eq for &BitcoinSystemRef {}

#[derive(Debug)]
pub struct BitcoinSystem {
...
impl BitcoinSystemRef {
    pub fn new_ref() -> BitcoinSystemRef {
        let sys = BitcoinSystem::new();
        BitcoinSystemRef(Rc::new(RefCell::new(sys)))
    }
    ...
    pub fn set_program_output(&self, var: &impl Bar) -> Result<()> {
...
impl BitcoinSystem {
    ...
    pub fn set_program_output(&mut self, var: &impl Bar) -> Result<()> {
        if self.finalized {
            return Err(Error::msg("The constraint system has been finalized"));
        }
